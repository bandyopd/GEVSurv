#################################################################################
## R code to run the h-likelihood estimation procedure for fitting the N (Normal) model
...
phi_temp = c(lambda_temp, alpha_temp)
...
while(1)
{
  ...
  for(i in 1:q)
  {
    res_v = optimize(f = hlik_vi_ftn, interval = c(-10, 10), maximum = TRUE, i=i, theta = theta_temp, dist = "N")
    
    v_new[i] = res_v$maximum
  }
  
  beta_temp = theta_temp[1:p]
  
  phi_temp = theta_temp[-(1:p)]
  
  res_beta = optim(par = beta_temp, fn = pvh_ftn, phi = phi_temp, v = v_new, dist = "N")
  
  beta_new = res_beta$par
  
  psi_new = c(beta_new, v_new)
  
  res_phi = optim(par = phi_temp, fn = ppsih_ftn, psi = psi_new, dist = "N")
...
lambda.hat = theta_hat[p+1]
alpha.hat = theta_hat[p+2]

se.beta.hat = beta_se_ftn(theta_hat, v_hat, dist = "N")
...
for(i in 1:q)
{
  ell1 = ell1 + ell1_i_ftn(v_hat[i], i, theta_hat, dist = "N")
}

source("df_ftn.R")

-2*ell1 + 2*df_ftn(theta_hat, v_hat, dist = "N")  #cAIC
